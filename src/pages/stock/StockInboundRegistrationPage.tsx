...
import { createManualInbound } from "@/api/stock/inbound.ts";
import { analyzeReceipt } from "@/api/ocr/ocr.ts";
import type { ManualInboundRequest, Item as OCRItem } from "@/types";
    ...
    const handleOCRClick = () => {
        ...
        input.onchange = async (e) => {
            ...
            try {
                ...
                if (result.vendor?.id?.value) {
                    setVendorId(String(result.vendor.id.value));
                }

                if (result.items && result.items.length > 0) {
                    const newItems: ItemDraft[] = result.items.map((it: OCRItem) => ({
                        id: newId(),
                        rawProductName: it.ingredient?.name?.value || "",
                        quantity: it.quantity?.value || "",
                        unitCost: it.costPrice?.value || "",
                        expirationDate: it.expirationDate?.value || "",
                    }));
                    setItems(newItems);
                } else {
    ...
    const handleSubmitRegister = async () => {
        ...
        try {
            ...
            if (response && response.inboundPublicId) {
                navigate(`/stock/inbound/${response.inboundPublicId}`);
            } else {
                navigate("/stock/inbound");
            }
        } catch (e) {
            console.error(e);
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className={`inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-black transition ${scanning
                                ? "text-gray-400 cursor-not-allowed"
                                : "text-gray-800 hover:bg-gray-50"
                                }`}
                        >
                            {scanning ? "스캔 중..." : "명세서 스캔 (OCR)"}
                        </button>
