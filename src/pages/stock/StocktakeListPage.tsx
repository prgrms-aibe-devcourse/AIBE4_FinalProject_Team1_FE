...
const StocktakeListPage = () => {
    ...
    const [sheets] = useState([
        {
            ...
            title: "2월 정기 재고 실사",
            status: "CONFIRMED",
            confirmedAt: "2024-02-15T10:00:00Z",
        ...
        },
        {
            ...
            title: "우유류 긴급 실사",
            status: "CONFIRMED",
            confirmedAt: "2024-02-20T15:30:00Z",
    ...
    const handleViewDetail = (sheetId: number) => {
        ...
        // 향후 상세 페이지 가기: navigate(`/stock/stocktakes/${sheetId}`);
    };
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case "CONFIRMED":
                return <span
                    className="px-2.5 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-bold">확정됨</span>;
            default:
                return <span
                    className="px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 text-xs font-bold">작성 중</span>;
        }
    };
                            ...
                            {sheets.map((sheet) => (
                                <tr key={sheet.sheetId} className="hover:bg-slate-50 transition-colors group">
                                    <td className="px-6 py-5">
                                        <div
                                            className="font-bold text-slate-800 group-hover:text-emerald-600 transition">{sheet.title}</div>
                                    ...
                                    <td className="px-6 py-5 text-sm text-slate-500">
                                        {sheet.confirmedAt ? (
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                {new Date(sheet.confirmedAt).toLocaleTimeString([], {
                                                    hour: '2-digit',
                                                    minute: '2-digit'
                                                })}
                                            </div>
